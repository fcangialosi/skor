...
int main(int argc, char *argv[]) {
	...
	// Set up the outgoing pipeline
	char pipedesc[1000];
	snprintf(pipedesc, sizeof(pipedesc), 
		"skorsrc name=skorsrc ! video/x-raw,format=YUY2,width=640,height=480,framerate=%u/1 ! v4l2sink device=%s",
	...
	if (args.end == ORIGIN)
		server_thread = g_thread_new("tunnelorigin", (GThreadFunc)tunnel_start_server_loop, &args);
	else
		server_thread = g_thread_new("tunnelterminus", (GThreadFunc)tunnel_end_server_loop, &args);
	...
	gst_object_unref(GST_OBJECT(inpipe));
	g_source_remove(out_watch_id);
	g_source_remove(in_watch_id);
